...
alter table public.tour_packages
  drop constraint if exists tour_packages_people_range;

alter table public.tour_packages
  add constraint tour_packages_people_range check (
...
create index if not exists idx_tour_packages_people
  on public.tour_packages (account_id, min_people, max_people);
