...
This document outlines the security controls, tenant isolation architecture, and vulnerability reporting procedures for Helpa.

---

## 1. Multi-Tenant Isolation Model

Tenant isolation is the core architectural invariant of Helpa:
...
SUPER ADMIN             ─►  PLATFORM-LEVEL GOVERNANCE (Explicit Server-Side Authorization)
```
...
- **Cross-Industry Isolation**: Workspaces are bounded by industry modules (*Health, Coaching, Solo Tutor, Salon, Real Estate*). A Health workspace cannot execute Salon or Real Estate operations.
...
- **Credentials at Rest**: WhatsApp access tokens and third-party secrets are encrypted with **AES-256-GCM** using a 256-bit encryption key (`ENCRYPTION_KEY`). GCM ensures ciphertext authenticity and integrity with a 16-byte authentication tag.
...
3. Relevant logs or request traces without including production customer data.
