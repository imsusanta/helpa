...
| `/api/metrics/observation`       | `GET`             | `viewer`           | `.eq('account_id', ctx.accountId)`                  |                    | Aggregates only. No raw events, no PII. Publication always `allowed: false` until operator sign-off. |
| `/api/pilot/readiness`           | `GET`             | `admin`            | Session account only                                |                    | Counts and connection flags. No tokens, phones, or patient rows.                                     |
