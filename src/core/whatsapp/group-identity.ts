...
/**
 * True if an address is a WhatsApp LID (Linked Device / Privacy ID).
 * LIDs are internal 14-16 digit numbers generated by WhatsApp multi-device companion sessions
 * that do not represent valid national E.164 phone numbers.
 */
export function isWhatsAppLidAddress(
  value: string | null | undefined
): boolean {
  ...
  // WhatsApp LIDs are typically 14-16 digit device IDs
  if (
    digits.length >= 14 &&
 ...
 * Rejects group IDs, broadcast JIDs, channel JIDs, LID device IDs, and corrupted numeric strings.
 */
export function isValidIndividualPhone(
  ...
  if (!raw) return false;
  if (
    isWhatsAppGroupAddress(raw) ||
    isWhatsAppGroupJid(raw) ||
    isWhatsAppLidAddress(raw) ||
    isWhatsAppLidJid(raw)
  ) {
    return false;
  }
  const digits = raw.replace(/\D/g, '');
 ...
 * Show them as E.164 with a leading +. Group/channel/LID ids stay blank.
 */
export function formatWhatsAppDisplayPhone(phone?: string | null): string {
  ...
  if (!isValidIndividualPhone(raw)) return '';
  const digits = raw.replace(/\D/g, '');
  if (!digits) return '';
  return `+${digits}`;
}
