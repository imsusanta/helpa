...
export async function getWhatsAppConnection(
  ...
  return {
    ...
    connectionStatus,
    coexistenceStatus: config.coexistence_status || 'unknown',
...
export async function sendWhatsAppMessage(
  ...
  try {
    ...
    return {
      success: false,
      error: 'WhatsApp access token could not be decrypted',
      timestamp: new Date().toISOString(),
    };
  }
...
export async function getWhatsAppHealth(
  ...
  if (conn.connectionStatus === 'PENDING_VERIFICATION') {
    webhookStatus = 'unregistered';
    issues.push('Webhook event delivery is pending registration.');
  }
  return {
