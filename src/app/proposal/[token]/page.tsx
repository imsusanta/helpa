...
export default function PublicTripProposalPage({
  params,
}: {
  ...
  useEffect(() => {
    params.then(({ token }) =>
      fetch(`/api/public/trip-proposals/${encodeURIComponent(token)}`, {
        cache: 'no-store',
      })
        .then(async (res) => {
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6">
        <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-xl">
          <header className="bg-slate-950 px-6 py-8 text-white sm:px-10 sm:py-10">
            <div className="text-xs font-bold tracking-[0.2em] text-emerald-300 uppercase">
              Trip Proposal · {proposal.quotation_number}
            </div>
          ...
          <div className="space-y-8 p-6 sm:p-10">
            <section className="grid gap-3 sm:grid-cols-4">
              {[
                ...
                ['Hotel', trip.hotel_category],
                ['Meals', trip.meal_plan],
              ].map(([label, value]) => (
                <div
                  key={label}
            ...
            <section>
              ...
              <div className="mt-4 space-y-3">
                {(trip.itinerary || []).map((day) => (
                  ...
                  >
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-emerald-50 text-xs font-extrabold text-emerald-700">
                      {day.day}
                    </div>
                    <div>
            ...
            <section className="rounded-3xl bg-slate-50 p-5 sm:p-6">
              ...
              <div className="mt-5 space-y-2 text-sm text-slate-500">
                <div className="flex justify-between">
                  ...
                  <span>{money(proposal.subtotal, proposal.currency)}</span>
                </div>
                {proposal.tax_amount > 0 && (
                  <div className="flex justify-between">
                    <span>Tax</span>
                    <span>{money(proposal.tax_amount, proposal.currency)}</span>
                  </div>
                )}
                {proposal.discount_amount > 0 && (
                  <div className="flex justify-between">
                    <span>Discount</span>
                    <span>
                      -{money(proposal.discount_amount, proposal.currency)}
                    </span>
                  </div>
                )}
              </div>
              <div className="mt-4 border-t border-slate-200 pt-4">
                ...
                <div className="mt-2 flex justify-between text-xs text-emerald-700">
                  <span>Advance</span>
                  <span>{money(trip.advance_amount, proposal.currency)}</span>
                </div>
                <div className="mt-1 flex justify-between text-xs font-bold text-slate-700">
                  <span>Balance</span>
                  <span>{money(trip.balance_amount, proposal.currency)}</span>
                </div>
              </div>
            </section>
            ...
            <p className="text-center text-[11px] text-slate-400">
              Prepared by your travel partner · Proposal valid until{' '}
              {date(proposal.valid_until)}
            </p>
          </div>
        </div>
      </div>
