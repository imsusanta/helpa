...
export default function LabReportsPage() {
  ...
  const [activeFilter, setActiveFilter] = useState<"all" | "processing" | "ready" | "collected">("all");
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const { data: reportRows } = await db
        ...
        .select("id, test_name, status, result_url, notes, created_at, patient:contacts(id, name, phone)")
  ...
  const handleCreateReport = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await db.from("hospital_lab_reports").insert({
        ...
        test_name: testName,
        status: status,
        result_url: resultUrl || `https://wacrmsusanta.vercel.app/demo-report.pdf`,
        notes: notes || null,
      });
  ...
  const handleUpdateStatus = async (reportId: string, newStatus: "processing" | "ready" | "collected") => {
    const db = createClient();
    try {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <select value={status} onChange={(e) => setStatus(e.target.value as any)} className="w-full flex h-10 rounded-md border border-input bg-background px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                <option value="processing">Processing (Sample Collected)</option>
                <option value="ready">Ready (Report Completed)</option>
              </select>
            </div>
            <div className="space-y-2">
              <Label>Report Document URL (Optional)</Label>
              <Input value={resultUrl} onChange={(e) => setResultUrl(e.target.value)} placeholder="Link to report PDF..." />
            </div>
            <div className="space-y-2 md:col-span-2">
              <Label>Lab Notes / Preliminary Findings</Label>
              <Input value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Add any details for doctors or patient..." />
            </div>
          </div>
      ...
      <div className="flex border-b border-border">
        {(["all", "processing", "ready", "collected"] as const).map((tab) => (
          <button
            key={tab}
                ...
                {filteredReports.map((rep) => (
                  <tr key={rep.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right space-x-1.5 flex justify-end items-center">
                      {rep.status === "ready" && rep.result_url && (
                        <a
                          href={rep.result_url}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-1 rounded-md border border-border bg-card hover:bg-muted text-xs py-1.5 px-3 font-semibold text-foreground cursor-pointer transition-colors"
                        >
                      ...
                      {rep.status === "processing" && (
                        ...
                        >
                          <Check className="h-3.5 w-3.5 mr-1" /> Mark Ready
                        </Button>
                      )}
                      {rep.status === "ready" && (
                        <Button
