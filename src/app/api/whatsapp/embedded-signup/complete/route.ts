...
function verifyState(state: string, accountId: string, userId: string) {
  ...
  const expected = createHmac('sha256', process.env.META_APP_SECRET || '').update(payload).digest('hex');
...
export async function POST(request: Request) {
  try {
    ...
    const { data: existing } = await db.from('whatsapp_config').select('id').eq('account_id', ctx.accountId).maybeSingle();
    const result = existing?.id
      ? await db.from('whatsapp_config').update(payload).eq('id', existing.id)
      : await db.from('whatsapp_config').insert({ ...payload, created_at: now });
