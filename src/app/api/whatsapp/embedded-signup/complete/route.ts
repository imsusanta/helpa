...
export async function POST(request: Request) {
  try {
    ...
    if (
      !body?.code ||
      !body.state ||
      !verifyState(body.state, ctx.accountId, ctx.userId)
    ) {
      return NextResponse.json(
        { error: 'Invalid or expired WhatsApp connection session' },
        { status: 400 }
      );
    }
    ...
    if (!tokenResponse.ok || !tokenData?.access_token) {
      console.error(
        '[WhatsApp signup token exchange]',
        tokenData?.error?.message || tokenResponse.status
      );
      return NextResponse.json(
        { error: 'Meta could not complete the WhatsApp connection' },
        { status: 400 }
      );
    }
    ...
    if (!wabaId && Array.isArray(whatsappScope?.target_ids)) {
      wabaId = String(whatsappScope.target_ids[0] || '');
    }

    if (!phoneNumberId && wabaId) {
      const phones = await fetch(
        `https://graph.facebook.com/${GRAPH_VERSION}/${encodeURIComponent(wabaId)}/phone_numbers?access_token=${encodeURIComponent(accessToken)}`
      );
    ...
    if (result.error) {
      console.error(
        '[WhatsApp signup persistence]',
        result.error.message
      );
      return NextResponse.json(
        {
          error:
            'WhatsApp connected with Meta but could not be saved to Helpa',
        },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      { error: 'Unable to complete WhatsApp connection' },
      { status: 500 }
    );
  }
}
