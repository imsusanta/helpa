...
export async function POST(request: Request) {
  try {
    ...
    const limit = checkRateLimit(`admin:ai-test:${ctx.userId}`, RATE_LIMITS.adminAction)
    ...
    if (!api_key || api_key.trim() === '' || api_key.includes('••••')) {
      ...
      if (error) {
        ...
        return NextResponse.json({ error: 'Failed to fetch saved API credentials' }, { status: 500 })
      }
      ...
      try {
        ...
        return NextResponse.json({ error: 'Failed to decrypt saved API Key' }, { status: 500 })
      }
    }
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'user',
            content: 'Respond with a single word confirming this is a test: "Success"',
          },
        ],
      }),
    })
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: `OpenRouter API Error: ${errorDetail}` }, { status: response.status })
    }
    ...
    if (!aiText) {
      return NextResponse.json({ error: 'Received empty response from OpenRouter' }, { status: 502 })
    }
