...
export async function GET() {
  try {
    ...
    const { data: account, error } = await ctx.supabase
      .from('accounts')
      .select('reminder_enabled, reminder_24h_enabled, reminder_2h_enabled, reminder_custom_time, reminder_template, reminder_business_hours')
      .eq('id', ctx.accountId)
      .single();
    ...
    return NextResponse.json({
      ...
      reminder_business_hours: account?.reminder_business_hours || { enabled: false, start: '09:00', end: '17:00' },
    });
  } catch (err) {
...
export async function PATCH(request: Request) {
  try {
    ...
    const limit = checkRateLimit(`admin:reminders-config:${ctx.userId}`, RATE_LIMITS.adminAction);
    ...
    if (typeof reminder_custom_time === 'number' || reminder_custom_time === null) {
    ...
    if (typeof reminder_business_hours === 'object' && reminder_business_hours !== null) {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to update reminder configuration' }, { status: 500 });
    }
    ...
    return NextResponse.json({
      ...
      reminder_business_hours: data?.reminder_business_hours || { enabled: false, start: '09:00', end: '17:00' },
    });
  } catch (err) {
    return toErrorResponse(err);
  }
