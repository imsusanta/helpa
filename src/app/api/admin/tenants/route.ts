...
export async function GET() {
  try {
    ...
    const [accountsResult, profilesResult, subscriptionsResult, contactsResult] =
      await Promise.all([
        database.from('accounts').select('*').order('created_at', { ascending: false }),
        ...
        subscription: subscription
          ? {
              ...
              plan: subscription.plan_slug
                ? {
                    id: String(subscription.plan_slug),
                    name: String(subscription.plan_slug),
                  }
                : null,
            }
          : null,
...
export async function PATCH(request: Request) {
  try {
    ...
    if (plan) {
      changes.plan_slug = plan.slug;
      changes.monthly_amount = plan.monthlyPrice;
      changes.setup_fee_amount = plan.setupFee;
      changes.currency = plan.currency;
    }
    ...
    if (existing) {
      ...
      if (!plan || !status) {
        return NextResponse.json(
          { error: 'A new subscription requires planId and status' },
          { status: 400 }
        );
      }
