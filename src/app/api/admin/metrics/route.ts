...
export async function GET() {
  try {
    ...
    const [accounts, contacts, profiles, subscriptions, usage] =
      await Promise.all([
        database.from('accounts').select('id', { count: 'exact', head: true }),
        database.from('contacts').select('id', { count: 'exact', head: true }),
        database.from('profiles').select('id', { count: 'exact', head: true }),
        database
          .from('subscriptions')
          .select('status, plan_slug, plan:plans(name)'),
        database
          .from('usage_tracking')
    ...
    assertResult(profiles.error, 'Failed to count profiles');
    ...
    for (const subscription of subscriptions.data || []) {
      ...
      const planName = String(
        (relation as Record<string, unknown> | null)?.name ||
