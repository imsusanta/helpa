...
export async function GET() {
  try {
    ...
    if (!settings.system_ai_provider) settings.system_ai_provider = 'openrouter';
    if (!settings.system_ai_fallback_provider) settings.system_ai_fallback_provider = 'none';
    if (!settings.system_openrouter_model) settings.system_openrouter_model = 'google/gemini-2.5-flash';
    if (!settings.system_orcarouter_model) settings.system_orcarouter_model = 'orcarouter/auto';
    if (settings.system_openrouter_enabled === undefined) settings.system_openrouter_enabled = 'true';
    if (settings.system_orcarouter_enabled === undefined) settings.system_orcarouter_enabled = 'true';

    // Environment fallback visibility flags
    if (settings.has_system_openrouter_api_key === undefined || settings.has_system_openrouter_api_key === false) {
      settings.has_system_openrouter_api_key = !!process.env.OPENROUTER_API_KEY;
    }
    if (settings.has_system_orcarouter_api_key === undefined || settings.has_system_orcarouter_api_key === false) {
...
export async function POST(req: Request) {
  try {
    ...
    if (typeof system_openrouter_enabled === 'string' || typeof system_openrouter_enabled === 'boolean') {
      upserts.push({ key: 'system_openrouter_enabled', value: String(system_openrouter_enabled) });
      auditActions.push('AI_PROVIDER_STATUS_CHANGED');
    }

    if (typeof system_orcarouter_enabled === 'string' || typeof system_orcarouter_enabled === 'boolean') {
      upserts.push({ key: 'system_orcarouter_enabled', value: String(system_orcarouter_enabled) });
      auditActions.push('AI_PROVIDER_STATUS_CHANGED');
    }

    if (available_models !== undefined) {
      const val = typeof available_models === 'string' ? available_models : JSON.stringify(available_models);
      upserts.push({ key: 'available_models', value: val });
      auditActions.push('AI_MODEL_UPDATED');
    }

    if (system_feature_routing !== undefined) {
      const val = typeof system_feature_routing === 'string' ? system_feature_routing : JSON.stringify(system_feature_routing);
    ...
    if (typeof system_openrouter_api_key === 'string') {
      ...
      if (trimmed.length > 0) {
        upserts.push({ key: 'system_openrouter_api_key', value: encrypt(trimmed) });
        auditActions.push('AI_API_KEY_UPDATED');
      } else if (system_openrouter_api_key === '') {
        await db.from('system_settings').delete().eq('key', 'system_openrouter_api_key').catch(() => {});
    ...
    if (typeof system_orcarouter_api_key === 'string') {
      ...
      if (trimmed.length > 0) {
        upserts.push({ key: 'system_orcarouter_api_key', value: encrypt(trimmed) });
        auditActions.push('AI_API_KEY_UPDATED');
      } else if (system_orcarouter_api_key === '') {
        await db.from('system_settings').delete().eq('key', 'system_orcarouter_api_key').catch(() => {});
        auditActions.push('AI_API_KEY_CLEARED');
      }
    }
    ...
    return NextResponse.json({ success: true, updated: upserts.map((u) => u.key) });
  } catch (err: unknown) {
