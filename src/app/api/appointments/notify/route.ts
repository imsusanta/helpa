...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!accountId) {
      return NextResponse.json(
        { error: 'FORBIDDEN', message: 'User is not associated with an account' },
        { status: 403 }
      )
    }
    ...
    const { data: appt, error: apptErr } = await db
      .from('appointments')
      .select('*, patient:contacts(id, name, phone, account_id), doctor:hospital_doctors(id, name)')
      .eq('id', appointmentId)
    ...
    if (apptErr || !appt) {
      return NextResponse.json(
        { error: 'APPOINTMENT_NOT_FOUND', message: 'Appointment not found or access denied' },
        { status: 404 }
      )
    }
    ...
    if (!patientPhone || !contactId || appt.patient?.account_id !== accountId) {
      return NextResponse.json(
        { error: 'PATIENT_NOT_FOUND', message: 'Patient contact details not found or access denied' },
        { status: 404 }
      )
    }
    ...
    if (!conv) {
      return NextResponse.json(
        { error: 'CONVERSATION_ERROR', message: 'Failed to find/create conversation' },
        { status: 500 }
      )
    }
    ...
    const token = generatePdfToken({ appointmentId: appt.id, accountId, expiresAt })
...
Please arrive 15 minutes before your time slot. Thank you!`
    ...
    console.error('[POST /api/appointments/notify] Failed to resend appointment slip:', err)
    return NextResponse.json(
      { error: 'APPOINTMENT_NOTIFY_FAILED', message: 'Failed to process notification request' },
      { status: 500 }
    )
  }
}
