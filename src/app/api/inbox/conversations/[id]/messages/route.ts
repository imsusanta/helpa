...
function normalizeMessage(doc: Record<string, unknown>): Message {
  return {
    ...
    sender_type: (doc.senderType ||
      doc.sender_type ||
      'customer') as string as SenderType,
    sender_id: (doc.senderId || doc.sender_id) as string | undefined,
    content_type: (doc.contentType ||
      doc.content_type ||
      'text') as string as ContentType,
    content_text: ((doc.contentText || doc.content_text) as string) || '',
    ...
    status: (doc.status || 'sent') as string as MessageStatus,
    created_at:
 ...
 *
 * Fetches messages for a conversation, verifying that the parent conversation
...
export async function GET(request: NextRequest, { params }: Params) {
  try {
    ...
    // 1. Verify parent conversation belongs to this tenant
