...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    try {
      payload = asRecord(JSON.parse(rawBody) as unknown);
    } catch {
    ...
    const providerPayload = asRecord(payload.payload);
    const payment = asRecord(asRecord(providerPayload.payment).entity);
    const order = asRecord(asRecord(providerPayload.order).entity);
    if (
    ...
    const notes = Object.keys(paymentNotes).length > 0 ? paymentNotes : orderNotes;
