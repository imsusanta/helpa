...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    return NextResponse.json(
      { data: tags || [], requestId: correlationId },
    ...
    return errorResponse(500, 'TAGS_FETCH_FAILED', correlationId);
  }
}
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (insertErr) {
      console.error('[tags] Insert failed:', insertErr);
      return errorResponse(500, insertErr.message, correlationId);
    }

    return NextResponse.json(
      { data: newTag, requestId: correlationId },
      {
        status: 201,
        headers: { ...PRIVATE_HEADERS, 'X-Request-Id': correlationId },
      }
    ...
    return errorResponse(500, 'TAG_CREATE_FAILED', correlationId);
  }
}
