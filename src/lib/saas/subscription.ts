...
export async function checkPlanLimits(
  ...
  if (error || !sub) {
    ...
    console.warn(`[SaaS Limits] No subscription found for account: ${accountId}. Allowing bypass.`);
    return true;
  }
  ...
  if (limitKey === "max_contacts") {
    ...
    if ((count ?? 0) >= limitValue) {
      throw new Error(`Plan contact limit of ${limitValue} exceeded. Please upgrade your plan.`);
    }
  }
  ...
  if (limitKey === "max_users") {
    ...
    if ((count ?? 0) >= limitValue) {
      throw new Error(`Plan team member limit of ${limitValue} exceeded. Please upgrade your plan.`);
    }
  }
  ...
  if (limitKey === "max_ai_requests") {
    ...
    if ((usage?.ai_requests ?? 0) >= limitValue) {
      throw new Error(`Monthly AI request limit of ${limitValue} exceeded. Please upgrade your plan.`);
    }
  }
...
export async function incrementUsage(
  ...
  if (error) {
    console.error(`[SaaS Usage] Failed to increment usage metric ${metric}:`, error);
  }
}
