...
export async function scheduleAppointmentReminders(input: AppointmentReminderInput): Promise<void> {
  ...
  for (const automation of automations ?? []) {
    ...
    let runAt: Date;
    try {
      runAt = appointmentLocalToUtc(input.appointmentDate, input.appointmentTime, config.timezone || DEFAULT_TIMEZONE, beforeMinutes);
    } catch (err) {
    ...
    if (pendingError) console.error('[automations] reminder scheduling failed:', pendingError);
  }
}

/** Cancel pending reminder rows for an appointment before rescheduling/cancellation. */
export async function cancelPendingAppointmentReminders(accountId: string, appointmentId: string): Promise<void> {
  const db = appwriteAdmin();
  ...
  if (error) console.error('[automations] cancelling pending reminders failed:', error);
}

/** Convert business-local appointment date/time to UTC and subtract N minutes. */
export function appointmentLocalToUtc(date: string, time: string, timeZone: string, beforeMinutes: number): Date {
  const [year, month, day] = date.split('-').map(Number);
  const [hour, minute = 0] = time.split(':').map(Number);
  if (!Number.isInteger(year) || !Number.isInteger(month) || !Number.isInteger(day) || !Number.isInteger(hour) || !Number.isInteger(minute) || hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    throw new Error('Invalid appointment date/time');
  }
  const guess = new Date(Date.UTC(year, month - 1, day, hour, minute));
  const offsetMinutes = getTimezoneOffsetMinutes(guess, timeZone);
  return new Date(guess.getTime() - offsetMinutes * 60_000 - beforeMinutes * 60_000);
}
