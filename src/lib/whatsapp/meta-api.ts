...
// Every Graph API call gets a hard timeout so a hung upstream request can
// never pin a route handler / worker indefinitely (outbox rows would stay
// 'processing' and callers would keep retrying with fresh idempotency keys).
const META_REQUEST_TIMEOUT_MS = 30_000;
// Media uploads/downloads move real bytes; give them longer.
const META_MEDIA_TIMEOUT_MS = 60_000;

/** fetch with a default per-request timeout; an explicit init.signal wins. */
function metaFetch(
  url: string | URL,
  init: RequestInit = {},
...
export async function sendTemplateMessage(
  ...
  for (const langCode of fallbackLangs) {
    ...
    try {
      ...
      const errMsg =
        errData.error?.message || `Meta API error: ${response.status}`;
 ...
 * Renders on WhatsApp as a card with an external link button `[ ↗ displayText ]`.
 */
export async function sendInteractiveCtaUrl(
  ...
  if (!displayText || !displayText.trim()) {
    throw new Error('Interactive CTA URL requires displayText.');
  }
  if (!url || !url.trim()) {
    throw new Error('Interactive CTA URL requires a valid url.');
  }
  ...
  const response = await metaFetch(targetUrl, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${accessToken}`,
