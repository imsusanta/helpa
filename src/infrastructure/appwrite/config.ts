function getValidEndpoint(): string {
  const envEndpoint = process.env.NEXT_PUBLIC_APPWRITE_ENDPOINT;
  if (
    !envEndpoint ||
    envEndpoint === 'https://cloud.appwrite.io/v1' ||
    envEndpoint.includes('localhost')
  ) {
...
function getValidProjectId(): string {
  ...
  if (
    !envProjectId ||
    envProjectId === 'wacrm_production' ||
    envProjectId.length < 15
  ) {
    return '6a79822b003adde92f63';
  }
  return envProjectId;
}

export const APPWRITE_CONFIG = {
  endpoint: getValidEndpoint(),
