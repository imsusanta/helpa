...
export function LandingHero({ isAuthenticated }: HeroProps) {
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-[#FAF9FC] via-[#F7F5FC] to-[#F1EEFA] pt-20 pb-8 sm:pt-24 lg:pt-28">
      <div className="pointer-events-none absolute inset-0" aria-hidden="true">
        <div className="absolute left-0 top-32 h-56 w-56 rounded-full bg-white/50 blur-3xl" />
        <div className="absolute left-1/2 top-52 h-96 w-96 -translate-x-1/2 rounded-full bg-[#CFC7FF]/20 blur-3xl" />
        <div className="absolute right-0 bottom-0 h-72 w-72 rounded-full bg-[#B4F73C]/10 blur-3xl" />
      </div>

      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-5xl text-center">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-emerald-300/70 bg-emerald-50/90 px-4 py-2 text-sm font-bold text-emerald-700 shadow-sm backdrop-blur">
            <span className="text-emerald-500">✦</span>
            AI-powered WhatsApp automation for businesses
          </div>

          <h1 className="mx-auto max-w-5xl text-5xl font-black leading-[0.96] tracking-tight text-[#110E3D] sm:text-6xl lg:text-[78px]">
            Automate WhatsApp.<br />
            Get <span className="relative inline-block bg-gradient-to-r from-[#14B884] via-[#25D1A7] to-[#14B6BC] bg-clip-text text-transparent">
              More Leads.
              <span className="pointer-events-none absolute -bottom-4 left-1/2 h-4 w-[96%] -translate-x-1/2 rotate-[-1.5deg] rounded-[50%] border-b-[4px] border-[#9D7CFF]" aria-hidden="true" />
            </span>
          </h1>

          <p className="mx-auto mt-6 max-w-3xl text-base leading-8 text-[#5C587D] sm:text-lg lg:text-xl">
          ...
          <div className="mt-8 flex flex-col items-center justify-center gap-4 sm:flex-row">
            <Link
              ...
              className="flex min-h-12 items-center gap-2 rounded-full bg-gradient-to-r from-[#B9F22F] via-[#7DE69B] to-[#4EE3C1] px-9 py-3.5 text-base font-extrabold text-[#110E3D] shadow-lg shadow-emerald-100/70 transition hover:-translate-y-0.5 hover:shadow-xl"
            >
          ...
          <div className="mx-auto mt-10 grid max-w-4xl gap-3 sm:grid-cols-3">
            {outcomes.map(({ icon: Icon, label }) => (
              <div key={label} className="flex items-center gap-3 rounded-xl border border-white/90 bg-white/85 p-4 text-left text-sm font-semibold text-[#4D4A6B] shadow-[0_8px_28px_rgba(17,14,61,0.05)] backdrop-blur">
        ...
        <div className="mt-14">
          <div className="mb-5 flex items-center justify-center gap-4 border-b border-slate-200/80 pb-2 sm:gap-10">
            {tabs.map(({ icon: Icon, label }, index) => (
              <button key={label} type="button" className={`flex items-center gap-2 border-b-2 px-3 pb-3 pt-2 text-sm font-bold sm:px-4 sm:text-base ${index === 0 ? 'border-[#110E3D] text-[#110E3D]' : 'border-transparent text-[#6B6886]'}`}>
          ...
          <div className="mx-auto overflow-hidden rounded-[28px] border border-indigo-100/70 bg-[#ECEAFC] p-3 shadow-[0_20px_60px_rgba(54,46,120,0.14)] sm:p-6">
            ...
            <div className="grid min-h-[440px] grid-cols-1 overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm lg:grid-cols-[220px_1fr_220px]">
              <div className="border-b border-slate-100 p-4 lg:border-b-0 lg:border-r">
                ...
                <div className="space-y-2">
                  <div className="rounded-xl border border-indigo-100 bg-[#F7F8FD] p-3"><div className="flex items-center justify-between"><span className="text-xs font-bold text-[#110E3D]">Rahul Sharma</span><span className="text-[10px] text-slate-400">10:15 AM</span></div><p className="mt-1 truncate text-[11px] text-slate-600">Interested in your product</p><div className="mt-2 flex gap-1"><span className="rounded-full bg-blue-100 px-1.5 py-0.5 text-[9px] font-semibold text-blue-700">Lead</span><span className="rounded-full bg-emerald-100 px-1.5 py-0.5 text-[9px] font-semibold text-emerald-700">Qualified</span></div></div>
