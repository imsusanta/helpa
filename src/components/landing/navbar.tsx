...
export function LandingNavbar({
  ...
  useEffect(() => {
    // Auth is intentionally non-blocking: the landing page can render from
    // static HTML while the CTA updates after the session check completes.
    let cancelled = false;
        ...
        <div className="hidden items-center gap-3 md:flex">
          {isAuthenticated ? (
            ...
            <>
              <Link
                href="/login"
                className="px-4 py-2 text-sm font-semibold text-[#110E3D] hover:text-[#16A34A]"
              >
                Log in
              </Link>
              <Link
