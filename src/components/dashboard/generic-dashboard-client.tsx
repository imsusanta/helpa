...
function Panel({ children, className = '' }: { children: ReactNode; className?: string }) {
  return <section className={`overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_2px_8px_rgba(15,23,42,0.045)] ${className}`}>{children}</section>;
}
  ...
  return (
    <div className="mx-auto max-w-[1400px] space-y-4 pb-2">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <h1 className="text-[30px] font-extrabold leading-tight tracking-[-0.035em] text-[#0f172a]">{greeting}, {userName}! <span aria-hidden>👋</span></h1>
      ...
      <div className="helpa-kpi-grid grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {metricCards.map((card, index) => {
          ...
          return (
            <Panel key={card.key} className={`relative ${index < 4 ? 'h-[148px]' : 'h-[180px]'} p-5`}>
              <div className="flex items-start justify-between gap-3 pr-1">
                <p className="truncate text-[13px] font-bold tracking-[0.075em] text-[#64748b]">{card.label}</p>
                {hasFilter && <FilterPill>{card.filter}</FilterPill>}
              </div>
              <div className="mt-5 text-[34px] font-extrabold leading-none tracking-tight text-[#0f172a]">{formatValue(value, isWallet)}</div>
              {isWallet && (
              ...
              <div className={`absolute right-5 ${index < 4 ? 'top-[67px]' : 'top-5'} flex h-12 w-12 items-center justify-center rounded-xl shadow-lg ${toneClasses[card.tone]}`}>
      ...
      <div className="helpa-panel-grid grid gap-4 xl:grid-cols-[1.05fr_1fr_1fr]">
          ...
          <div className="space-y-5 p-5">
            {[
              ...
              <div key={label as string} className="flex items-center gap-3">
                ...
                <div className="h-8 flex-1 overflow-hidden rounded-lg bg-slate-50"><div className="h-full rounded-lg bg-slate-100" style={{ width: `${Math.min(100, Number(value) * 10)}%` }} /></div>
                <span className="w-8 text-right text-sm font-extrabold text-slate-800">{value}</span>
              </div>
            ))}
            <div className="flex items-center justify-end border-t border-slate-100 pt-4 text-sm font-bold text-slate-500">Total Pipeline Leads: <span className="ml-1 text-slate-800">{totalPipeline}</span></div>
          </div>
        </Panel>
        ...
        <Panel className="h-[360px]">
          ...
          <div className="flex h-[308px] flex-col justify-between p-5">
            <div className="flex flex-1 items-center justify-center"><div className="text-center"><div className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Total</div><div className="mt-1 text-3xl font-extrabold text-slate-900">{totalPipeline}</div></div></div>
        ...
        <Panel className="h-[360px]">
          <div className="flex h-[52px] items-center justify-between border-b border-slate-100 px-5">
            ...
            <div className="flex items-center gap-2"><FilterPill><Calendar className="h-3.5 w-3.5" />All Days</FilterPill><Link href="/follow-ups" className="text-xs font-bold text-blue-600">View All</Link></div>
          </div>
          <div className="flex h-[308px] items-center justify-center px-5 text-center">
            <div>
              <div className="mx-auto mb-3 flex h-11 w-11 items-center justify-center rounded-xl bg-slate-50 text-slate-400"><Calendar className="h-5 w-5" /></div>
              <p className="max-w-[300px] text-sm font-medium leading-5 text-slate-500">No upcoming call or meeting follow-ups for this period</p>
            </div>
          </div>
        </Panel>
      </div>
