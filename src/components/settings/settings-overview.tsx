  ...
  const tiles: {
    ...
    },
    {
      ...
      subtitle:
        counts?.members == null
          ? 'View team members'
          : `${counts.members} member${counts.members === 1 ? '' : 's'}${counts.pendingInvites ? ` · ${counts.pendingInvites} pending invite${counts.pendingInvites === 1 ? '' : 's'}` : ''}`,
    },
    {
      section: 'templates',
      loading: countsLoading,
      subtitle:
        counts?.templates == null
          ...
          : `${counts.templates} template${counts.templates === 1 ? '' : 's'}${counts.templatesPending ? ` · ${counts.templatesPending} pending review` : ''}`,
    },
    {
      section: 'deals',
      loading: false,
    ...
    },
    {
      ...
      subtitle:
        counts?.tags == null && counts?.customFields == null
          ...
          : `${counts?.tags ?? 0} tag${counts?.tags === 1 ? '' : 's'} · ${counts?.customFields ?? 0} custom field${counts?.customFields === 1 ? '' : 's'}`,
    },
    {
      section: 'appearance',
      loading: false,
            ...
            <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-7 py-5">
              ...
              {installationStep === 'idle' && (
                <button
                  ...
                  aria-label="Close"
                  className="shrink-0 rounded-full border border-slate-200 p-2 text-slate-500 transition hover:bg-slate-50 hover:text-slate-900"
                >
                  <X className="h-5 w-5" />
                </button>
              )}
            ...
            {installationStep === 'idle' ? (
              <>
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            ind.border,
                            isSelected
                              ? 'border-emerald-400 shadow-[0_8px_24px_rgba(16,185,129,0.12)] ring-2 ring-emerald-500'
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div
                              className={cn(
                                'flex h-10 w-10 items-center justify-center rounded-xl border bg-white shadow-sm',
                                ind.color,
                                'border-slate-200/80'
                              )}
                            >
                          ...
                          <div>
                            <h4 className="text-[16px] leading-5 font-bold text-slate-900">
                              {ind.name}
                            </h4>
                          ...
                          <div className="mt-auto border-t border-slate-200/80 pt-3">
                            <div className="flex flex-wrap gap-1.5">
                              {ind.features.slice(0, 3).map((feat) => (
                                <span
                                  key={feat}
                                  className="rounded-md border border-slate-200 bg-white px-2 py-1 text-[10px] leading-none font-semibold text-slate-500"
                                >
                                  {feat}
                                </span>
                              ))}
                ...
                <div className="flex flex-col gap-4 border-t border-slate-200 bg-slate-50/70 px-7 py-4 sm:flex-row sm:items-center sm:justify-between">
                  <p className="max-w-[560px] text-[11px] leading-4 text-slate-500 italic">
                  ...
                  <div className="flex shrink-0 items-center justify-end gap-3">
                    ...
                    <Button
                      ...
                      className="min-w-[190px] rounded-xl bg-indigo-600 px-5 font-semibold text-white shadow-sm hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50"
                    >
              ...
              <div className="mx-auto w-full max-w-md space-y-5 px-7 py-10 text-center">
                ...
                <div className="space-y-3 rounded-2xl border border-slate-200 bg-slate-50 p-5 text-left">
                      ...
                      <span
                        className={cn(
                          ...
                          item.status === 'done'
                            ? 'text-slate-900'
                            : item.status === 'loading'
                              ? 'animate-pulse font-bold text-indigo-600'
                              : 'text-slate-500'
                        )}
                      >
                        {item.label}
