  ...
  return (
    <section className="max-w-4xl animate-in fade-in-50 duration-200">
      ...
      <div className="flex border-b border-border mb-6 overflow-x-auto gap-2">
        {["all", "faq", "service", "pricing", "policy", "company"].map((tab) => (
          <button
            key={tab}
          ...
          >
            {tab === "all" ? "All Categories" : CATEGORY_MAP[tab as KbEntry["category"]].label}
          </button>
        ))}
      </div>
        ...
        <CardContent>
          {filteredEntries.length === 0 ? (
            <div className="text-center py-12 border border-dashed border-border rounded-lg">
              <Database className="size-8 text-muted-foreground mx-auto mb-2 opacity-50" />
              <p className="text-sm text-muted-foreground">
                No entries found in this category.
              </p>
              {canSendMessages && (
                <Button variant="link" onClick={handleOpenAddDialog} className="mt-2 text-primary">
              ...
              <Table>
                <TableHeader className="bg-muted/40">
                  <TableRow>
                    ...
                    <TableHead className="w-[260px]">Title / Question</TableHead>
                    <TableHead>Content / Answer</TableHead>
                    {canSendMessages && <TableHead className="w-[100px] text-right">Actions</TableHead>}
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filteredEntries.map((entry) => {
                    ...
                    return (
                      <TableRow key={entry.id} className="hover:bg-muted/30">
                        <TableCell>
                          <Badge variant="outline" className={`${catMeta.color} flex items-center gap-1 w-fit`}>
                        ...
                        <TableCell className="text-muted-foreground text-sm whitespace-pre-wrap max-w-sm truncate lg:max-w-none">
