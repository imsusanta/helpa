  ...
  const sidebar = (
    <aside className="flex h-full w-64 flex-col border-r border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-950">
      ...
      >
        {groups.map((group) => (
          <div key={group.label}>
            <p className="mb-2 px-3 text-[11px] font-semibold tracking-wider text-neutral-400 uppercase">
              {group.label}
            </p>
              ...
              {group.items.map((item) => {
                ...
                return item.href ? (
                  ...
                  <span
                    key={item.label}
                    className={classes}
                    title="This section will appear when existing data support is available"
                  >
                    <Icon className="h-4 w-4" />
                    {item.label}
      ...
      <button className="m-3 flex min-h-14 items-center gap-3 rounded-xl border border-neutral-200 p-3 text-left hover:bg-neutral-50 dark:border-neutral-800 dark:hover:bg-neutral-900">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-medium">Susanta Lohar</p>
    ...
    <>
      ...
      <div className="sticky top-0 z-30 -mx-4 mb-6 flex h-16 items-center gap-3 border-b border-neutral-200 bg-white/95 px-4 backdrop-blur lg:hidden dark:border-neutral-800 dark:bg-neutral-950/95">
        ...
        <div className="flex-1">
          <p className="text-sm font-semibold">Helpa Studio</p>
          <p className="text-xs text-neutral-500">Super Admin</p>
        </div>
        {onRefresh && (
          <Button
      ...
      <div className="fixed top-4 right-6 z-20 hidden items-center gap-2 lg:flex">
        <div className="relative">
          ...
          <input
            ...
            className="h-10 w-72 rounded-lg border border-neutral-200 bg-white pr-3 pl-9 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/15 dark:border-neutral-800 dark:bg-neutral-950"
          />
        </div>
        {onRefresh && (
          <Button
