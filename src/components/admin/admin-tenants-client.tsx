...
export function AdminTenantsClient() {
  ...
  const [sortBy, setSortBy] = useState<'newest' | 'oldest' | 'name' | 'plan'>(
    'newest'
  );
  ...
  const filteredAndSortedTenants = useMemo(() => {
    ...
    const filtered = tenants.filter((t) => {
      ...
      if (q) {
        ...
        const ownerNameMatch = t.owner?.full_name?.toLowerCase().includes(q);
        const ownerEmailMatch = t.owner?.email.toLowerCase().includes(q);
  ...
  }, [tenants, searchQuery, statusFilter, industryFilter, planFilter, sortBy]);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, statusFilter, industryFilter, planFilter, sortBy]);
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // SUBSCRIPTION MANAGEMENT MODAL HANDLERS
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  async function handleSubSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (response.ok) {
        toast.success('Business subscription updated');
  ...
  // Quick Action: Toggle Suspend / Reactivate
  async function handleToggleSuspend(tenant: Tenant) {
    const isCurrentlyActive = tenant.subscription?.status === 'active';
    ...
    try {
      ...
      if (res.ok) {
        toast.success(`Business ${actionName.toLowerCase()}d successfully`);
        loadData();
      } else {
        toast.error(`Failed to ${actionName.toLowerCase()} business`);
      }
    } catch {
  ...
  // View Business Detail Modal
  function handleOpenDetail(tenant: Tenant) {
  ...
  return (
    <AdminNav onRefresh={loadData} loading={loading}>
      <div className="space-y-6">
        ...
        {/* ═══════════════════════════════════════════════════════════════ */}
        {/* 3. BUSINESSES LIST — DESKTOP TABLE & MOBILE CARDS */}
        {/* ═══════════════════════════════════════════════════════════════ */}
        <div className="border-border/60 bg-card/80 overflow-hidden rounded-[1.35rem] border shadow-[0_18px_50px_-30px_rgba(15,23,42,0.45)]">
          {error ? (
            ...
            <div className="p-12 text-center">
              <Building2 className="text-muted-foreground/40 mx-auto h-10 w-10" />
              <p className="text-foreground mt-3 text-sm font-semibold">
                {hasActiveFilters ? 'No businesses found' : 'No businesses yet'}
              </p>
              <p className="text-muted-foreground mt-1 text-xs">
                {hasActiveFilters
                  ? 'Try a different business name, owner, or filter.'
                    ...
                    <div key={t.id} className="space-y-3 p-4">
                      ...
                      <div className="bg-muted/20 grid grid-cols-2 gap-2 rounded-xl p-2.5 text-[11px]">
                        ...
                        <div className="text-muted-foreground col-span-2">
                          Joined: {formatDate(t.created_at)} • Owner:{' '}
              ...
              <div className="space-y-4 py-2 text-xs">
                  ...
                  <div className="text-muted-foreground grid grid-cols-2 gap-2 pt-1">
                    <div>
                      ...
                      <span className="text-foreground font-medium">
                        {viewingTenant.owner?.full_name || 'Account Owner'}
                      </span>
                    </div>
                    <div>
                    ...
                    <div className="col-span-2">
                      ...
                      <span className="text-foreground font-medium">
                        {viewingTenant.owner?.email || 'N/A'}
                      </span>
                    </div>
                    <div>
                      <span className="text-[11px]">Team Members:</span>{' '}
                      <span className="text-foreground font-medium">
                        {viewingTenant.membersCount || 1}
                      </span>
                    </div>
                    <div>
                      <span className="text-[11px]">Contacts:</span>{' '}
                      <span className="text-foreground font-medium">
                        {viewingTenant.contactsCount || 0}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="border-border/60 bg-muted/20 space-y-2 rounded-xl border p-3.5">
                  ...
                  <div className="text-muted-foreground grid grid-cols-2 gap-2 pt-1">
                    ...
                    <div>
                      <span className="text-[11px]">Renewal Date:</span>{' '}
                      <span className="text-foreground font-medium">
              ...
              <DialogFooter className="flex items-center justify-between gap-2 pt-2 sm:justify-between">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    handleOpenSubDialog(viewingTenant);
                  }}
                  className="h-8 rounded-lg text-xs"
                >
                  Manage Subscription
