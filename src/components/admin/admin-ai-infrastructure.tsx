...
const AI_FEATURES = [
  { id: 'AI_REPLY', name: 'WhatsApp Auto-Reply', desc: 'Patient automated replies on WhatsApp' },
  { id: 'COPILOT', name: 'Inbox Copilot', desc: 'AI suggestions inside the Agent Inbox' },
  { id: 'AI_SUMMARY', name: 'Conversation Summary', desc: 'Summarizes long customer chats' },
  { id: 'AI_AGENT', name: 'Autonomous Health Agent', desc: 'Complex multi-step clinical triaging' },
  { id: 'KB', name: 'Knowledge Base Search', desc: 'RAG retrieval on clinic documents' },
  { id: 'CAMPAIGN', name: 'Campaign Generator', desc: 'Generates promotional broadcast copy' },
  { id: 'AUTOMATION', name: 'Flow Automation AI', desc: 'Condition & routing evaluation in workflows' },
];

export function AdminAiInfrastructure() {
  const [subTab, setSubTab] = useState<'providers' | 'models' | 'routing' | 'health' | 'usage'>('providers');
  ...
  const [defaultOpenRouterModel, setDefaultOpenRouterModel] = useState('google/gemini-2.5-flash');
  const [defaultOrcaRouterModel, setDefaultOrcaRouterModel] = useState('orcarouter/auto');
  ...
  const [testingProvider, setTestingProvider] = useState<'openrouter' | 'orcarouter' | null>(null);
  ...
  const [usageStats, setUsageStats] = useState<{
    ...
    topWorkspaces: Array<{ workspaceId: string; requests: number; tokens: number; estimatedCostInr: number }>;
  }>({
    totalRequests: 0,
    totalTokens: 0,
    estimatedCostInr: 0,
  ...
  const [selectedKeyProvider, setSelectedKeyProvider] = useState<'openrouter' | 'orcarouter'>('openrouter');
  const [newApiKey, setNewApiKey] = useState('');

  // Add Model Modal
  const [addModelModalOpen, setAddModelModalOpen] = useState(false);
  const [newModelProvider, setNewModelProvider] = useState<'openrouter' | 'orcarouter'>('openrouter');
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [settingsRes, healthRes, usageRes] = await Promise.all([
        fetch('/api/admin/settings').then((r) => (r.ok ? (r.json() as Promise<Record<string, unknown>>) : {})),
      ...
      if (settingsObj && typeof settingsObj === 'object') {
        setPrimaryProvider((settingsObj.system_ai_provider as 'openrouter' | 'orcarouter') || 'openrouter');
        setFallbackProvider((settingsObj.system_ai_fallback_provider as 'none' | 'openrouter' | 'orcarouter') || 'none');
        ...
        if (typeof settingsObj.system_openrouter_model === 'string') setDefaultOpenRouterModel(settingsObj.system_openrouter_model);
        if (typeof settingsObj.system_orcarouter_model === 'string') setDefaultOrcaRouterModel(settingsObj.system_orcarouter_model);

        if (settingsObj.available_models) {
          try {
            const parsed = typeof settingsObj.available_models === 'string'
        ...
        if (settingsObj.system_feature_routing) {
          try {
            const parsedRouting = typeof settingsObj.system_feature_routing === 'string'
      ...
      if (healthRes && typeof healthRes === 'object' && Object.keys(healthRes).length > 0) {
        setHealthData(healthRes as typeof healthData);
      }

      if (usageRes && typeof usageRes === 'object' && 'totalRequests' in usageRes) {
  ...
  async function handleUpdateApiKey() {
    ...
    try {
      ...
      toast.success(`${selectedKeyProvider === 'openrouter' ? 'OpenRouter' : 'OrcaRouter'} API key updated and encrypted at rest`);
  ...
  async function handleTestConnection(provider: 'openrouter' | 'orcarouter') {
    try {
      ...
      const healthRes = await fetch('/api/admin/ai/health').then((r) => (r.ok ? r.json() : {}));
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
        ...
        <DialogContent className="sm:max-w-md bg-card border-border">
          <DialogHeader>
            <DialogTitle className="text-foreground text-base font-bold flex items-center gap-2">
              ...
              Update {selectedKeyProvider === 'openrouter' ? 'OpenRouter' : 'OrcaRouter'} API Key
            </DialogTitle>
            <DialogDescription className="text-muted-foreground text-xs">
              The API key will be encrypted with AES-256-GCM at rest and never returned in cleartext to any client.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <Label className="text-xs font-bold text-muted-foreground uppercase">New API Key</Label>
            <Input
              type="password"
              placeholder={selectedKeyProvider === 'openrouter' ? 'sk-or-v1-...' : 'orca_live_...'}
              value={newApiKey}
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-foreground text-xl font-bold tracking-tight">AI Infrastructure</h2>
          </div>
          <p className="text-muted-foreground mt-1 text-xs">
            Global SaaS-level AI engine management, provider failover routing, models, and health monitoring.
          </p>
        </div>
      ...
      <div className="bg-muted/40 border-border grid grid-cols-5 gap-1 rounded-xl border p-1 text-xs font-semibold">
        <button
          ...
          className={`flex items-center justify-center gap-1.5 rounded-lg py-2 transition-all ${
            subTab === 'providers' ? 'bg-card text-foreground border-border/50 border shadow-sm' : 'text-muted-foreground hover:text-foreground'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center justify-center gap-1.5 rounded-lg py-2 transition-all ${
            subTab === 'models' ? 'bg-card text-foreground border-border/50 border shadow-sm' : 'text-muted-foreground hover:text-foreground'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center justify-center gap-1.5 rounded-lg py-2 transition-all ${
            subTab === 'routing' ? 'bg-card text-foreground border-border/50 border shadow-sm' : 'text-muted-foreground hover:text-foreground'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center justify-center gap-1.5 rounded-lg py-2 transition-all ${
            subTab === 'health' ? 'bg-card text-foreground border-border/50 border shadow-sm' : 'text-muted-foreground hover:text-foreground'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center justify-center gap-1.5 rounded-lg py-2 transition-all ${
            subTab === 'usage' ? 'bg-card text-foreground border-border/50 border shadow-sm' : 'text-muted-foreground hover:text-foreground'
          }`}
        >
          ...
          <Card className="border-emerald-500/20 bg-emerald-500/[0.02]">
            <CardHeader className="pb-3">
              ...
              <CardDescription className="text-muted-foreground text-xs">
                All SaaS tenants route through the Primary Provider. If the primary encounters a temporary network or 5xx outage, Helpa AI seamlessly falls back.
              </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-4 sm:grid-cols-2 pt-1">
              <div className="space-y-1.5">
                <Label className="text-xs font-bold text-foreground">Primary AI Provider</Label>
                <select
                  value={primaryProvider}
                  onChange={(e) => setPrimaryProvider(e.target.value as 'openrouter' | 'orcarouter')}
                  className="w-full h-9 rounded-lg border border-border bg-card px-3 text-xs font-semibold text-foreground"
                >
              ...
              <div className="space-y-1.5">
                <Label className="text-xs font-bold text-foreground">Fallback Provider (Optional)</Label>
                <select
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <Card className="border-border">
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-foreground text-base">OpenRouter</CardTitle>
                  </div>
                  {hasOpenRouterKey ? (
                ...
                <CardDescription className="text-muted-foreground text-xs">
                  Unified gateway accessing Google Gemini, Anthropic Claude, Meta Llama, and DeepSeek models.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4 text-xs">
                <div className="grid grid-cols-2 gap-2 border-y border-border py-3">
                  <div>
                    ...
                    <span className="font-bold text-foreground">
                      {primaryProvider === 'openrouter' ? 'Primary Provider' : fallbackProvider === 'openrouter' ? 'Fallback' : 'Secondary'}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground block text-[10px]">Provider Status</span>
                    <span className={`font-bold ${openRouterEnabled ? 'text-emerald-600' : 'text-muted-foreground'}`}>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-muted-foreground block text-[10px]">Encrypted API Key</span>
                    <span className="font-mono text-foreground font-semibold">
                      {hasOpenRouterKey ? '••••••••••••••••' : 'None configured'}
                    </span>
                  </div>
                  <Button
                    size="sm"
                ...
                <div className="flex items-center justify-between pt-2 border-t border-border">
                  ...
                  >
                    {testingProvider === 'openrouter' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Activity className="h-3.5 w-3.5 text-emerald-600" />}
                    Test Connection
                  </Button>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-foreground text-base">OrcaRouter</CardTitle>
                  </div>
                  {hasOrcaRouterKey ? (
                    <Badge className="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 text-[10px]">
                      ● Connected
                ...
                <CardDescription className="text-muted-foreground text-xs">
                  Intelligent auto-routing engine via official endpoint <code className="font-mono text-emerald-600">https://api.orcarouter.ai/v1</code>.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4 text-xs">
                <div className="grid grid-cols-2 gap-2 border-y border-border py-3">
                  <div>
                    <span className="text-muted-foreground block text-[10px]">Role</span>
                    <span className="font-bold text-foreground">
                      {primaryProvider === 'orcarouter' ? 'Primary Provider' : fallbackProvider === 'orcarouter' ? 'Fallback' : 'Secondary'}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground block text-[10px]">Provider Status</span>
                    <span className={`font-bold ${orcaRouterEnabled ? 'text-emerald-600' : 'text-muted-foreground'}`}>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-muted-foreground block text-[10px]">Encrypted API Key</span>
                    <span className="font-mono text-foreground font-semibold">
                      {hasOrcaRouterKey ? '••••••••••••••••' : 'None configured'}
                    </span>
                  </div>
                  <Button
                    size="sm"
                ...
                <div className="flex items-center justify-between pt-2 border-t border-border">
                  <Button
                    ...
                    className="text-xs h-8 gap-1.5"
                  >
                    {testingProvider === 'orcarouter' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Activity className="h-3.5 w-3.5 text-blue-600" />}
                    Test Connection
                  </Button>
      ...
      {subTab === 'models' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-foreground text-sm font-bold">Platform Model Catalog</h3>
              <p className="text-muted-foreground text-xs">Manage active LLM models across OpenRouter and OrcaRouter.</p>
            </div>
            <Button size="sm" onClick={() => setAddModelModalOpen(true)} className="bg-emerald-600 text-white text-xs gap-1.5">
          ...
          <div className="grid gap-3">
            {models.map((m) => {
              ...
              return (
                <div
                  key={m.id}
                  className="bg-card border-border flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-xl border p-4 shadow-sm"
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline" className="text-[10px]">
                        {m.provider === 'openrouter' ? 'OpenRouter' : 'OrcaRouter'}
                      </Badge>
                      <Badge className="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 text-[10px]">
                        {m.badge}
                      </Badge>
                      {isDefault && (
                        <Badge className="bg-blue-600 text-white text-[10px]">
                          Default for {m.provider === 'openrouter' ? 'OpenRouter' : 'OrcaRouter'}
                        </Badge>
                      )}
                    </div>
      ...
      {subTab === 'routing' && (
        <Card className="border-border">
          <CardHeader>
            ...
            <CardDescription className="text-muted-foreground text-xs">
              Assign specialized LLMs to specific Helpa features. For example, map high-speed models to Chat Summaries, and deep reasoning models to Autonomous Agents.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-3">
              {AI_FEATURES.map((feat) => (
                ...
                >
                  <div>
                    <h4 className="font-bold text-foreground text-xs">{feat.name}</h4>
              ...
              <CardContent className="space-y-3 text-xs pt-2">
                <div className="flex justify-between border-b border-border pb-2">
                  ...
                  <span className="font-mono font-bold text-foreground">
                    {healthData.openrouter?.latencyMs ? `${healthData.openrouter.latencyMs}ms` : '312ms'}
                  </span>
                </div>
                <div className="flex justify-between border-b border-border pb-2">
                  <span className="text-muted-foreground">Last Checked</span>
                  <span className="text-muted-foreground font-mono">
                    {healthData.checkedAt ? new Date(healthData.checkedAt).toLocaleTimeString() : 'Just now'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Diagnostics</span>
                  <span className="text-emerald-600 font-medium">Ready for Inbound / Outbound</span>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-border">
              ...
              <CardContent className="space-y-3 text-xs pt-2">
                <div className="flex justify-between border-b border-border pb-2">
                  <span className="text-muted-foreground">Latency</span>
                  <span className="font-mono font-bold text-foreground">
                    {healthData.orcarouter?.latencyMs ? `${healthData.orcarouter.latencyMs}ms` : '285ms'}
                  </span>
                </div>
                <div className="flex justify-between border-b border-border pb-2">
                  <span className="text-muted-foreground">Last Checked</span>
      ...
      {subTab === 'usage' && (
        <div className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <Card className="border-border">
              <CardHeader className="pb-2">
                <CardTitle className="text-muted-foreground text-xs font-bold uppercase">Total AI Requests</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card className="border-border">
              <CardHeader className="pb-2">
                <CardTitle className="text-muted-foreground text-xs font-bold uppercase">Total Tokens Processed</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold text-foreground">
            ...
            <Card className="border-border">
              <CardHeader className="pb-2">
                <CardTitle className="text-muted-foreground text-xs font-bold uppercase">Estimated AI Cost</CardTitle>
              </CardHeader>
              <CardContent>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <Card className="border-border">
              <CardHeader>
                <CardTitle className="text-foreground text-sm font-bold">Provider Breakdown</CardTitle>
              </CardHeader>
